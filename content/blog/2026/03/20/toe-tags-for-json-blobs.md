...
:blog-post/published #time/ldt "2026-03-20T16:30:00"

:blog-post/description

How do we deal with schemaless JSON documents in applications that continually evolve? I propose attaching a toe tag to each document, linking it to an explicit and immutable schema.
...
For a particular customer value, that we may colloquially refer to as "John", this will yield the following JSON when serialized: 
...
Now assume that time passes and that our application evolves, as applications are wont to do. At some new point in time, t<sub>1</sub>, we decide to add a new property to this class. It is a _display name_ that the customer is free to change at will! So now the class looks like this.
...
## Revenge of the schema

Yes, I know what you're thinking. This puts us in the hilarious situation of having "schemaless persistence, but also btw here is the schema". There are a number of benefits to this "bring your own schema" approach though. We retain most of the flexibility of the schemaless approach. We can evolve the schema without immediately migrating data unless we want to. We can do it gradually, at our leisure. We are not confined to a single schema at a time. At the same time we have proper contracts for our documents, which tells us what it means for them to be valid.
...
To summarize, the invariant _no bodies in the morgue without a toe tag_ is more important to me than the decoupling of bodies and tags. You could argue that it's more than a toe tag, it's like scribbling on the body itself, but that's OK with me. If that's what it takes, I'll do it.
...
This is nice, because it gives us all we need to reproduce the validation result if we should want to. (Omitting the fact that you can pass various evaluation options to the validation tool.)
