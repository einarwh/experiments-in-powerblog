...
But of course, time t<sub>2</sub> is not perpetuity. Time moves on, and t<sub>2</sub> will eventually be superceded by time t<sub>3</sub> and t<sub>4</sub> and many more points in time after that. Will things still work? It's hard to tell. We may not run into trouble ever, or there may simply be some delay before shit hits the fan. For instance, it may be that our UX component gladly renders null as an empty string. Great. But if at t<sub>3</sub>, someone decides that it would look better to uppercase the display name before rendering it, suddenly we are dereferencing null. Oops! 
...
Having an explicit JSON Schema be the contract has a number of benefits. It decouples the contract of the JSON document from any particular application. All you need to check that the document is valid is the document, the schema and a tool of your choice. It's worth bearing in mind that some tools interpret the various JSON Schema draft specifications differently though. This means that different tools may actually disagree on whether or not a given document satisfies the schema, as in the case of the "email" format above. In practice this is only a problem if you actually use more than one validation tool.
...
There are two main approaches that I can think of. Either the association is external to the JSON document, that is, outside the document somewhere, or it is internal, that is, embedded within the document. Each approach has some pros and cons, and each can be done in more than one way.

Keeping the link (between the document, the schema and the validation) external to the document has the benefit that it is very clean, very decoupled, very flexible. We can even pull out the honorary designation _orthogonal_ to describe how the document and its contract relate to each other. Tying a tag to the document's toe isn't the right metaphor for this though. It's more like maintaining a ledger, a log where you list the names of documents and the results of validating them against schemas. Hence I call this the ledger approach. 
...
What are the pros and cons of this approach? The ledger is completely decoupled from the documents it describes. Any given document could in principle be validated against zero, one or more schemas. Throw the ledger away, and the document remains unfazed.

A benefit of the ledger approach is that all the information about schemas and validation can be viewed in the same space. A drawback is that you must know about the ledger in order to find it! A developer unfamiliar with the solution may not know that there is any schema validation going on at all. They will look at the blob and see... JSON. If the ledger is just entries in a log, it means that a developer will have to sift and sort through it when they want to see the associations between documents and schemas, as well as the validation results.

A tempting variation may be to use any built-in support for metadata that the persistance technology may have. For instance, Azure Blob Storage allows you to associate _user defined metadata_ in the form of key-value pairs with any given blob. You could for instance define a "schema" property, the value of which is the ID or URL to your schema, e.g. "/myschemas/schema-20260319.json". You could also define a "valid" property with "true" and "false" as the obvious values.
...
How would you go about doing that? When you serialize a type in a static language like C#, the JSON property typically matches the name of the C# property, but $schema isn't a valid name for a C# property. You could potentially go through some hoops to make your JSON serializer spit out a property named $schema nevertheless. Many serializers offer some way of overriding the name of a property to whatever you want, for instance through custom attributes.

A much more direct approach is to add the metadata after serialization. For instance, you could do something like this:

[code]

There may be more clever and performant ways of doing this, by integrating more tightly with the serializer library. But this is easy to understand and it works. It suffices to illustrate the point. 
...
But does the presence of the 

What happens on deserialization? You need to choose a strategy. The only thing you have gained is explicit information. 